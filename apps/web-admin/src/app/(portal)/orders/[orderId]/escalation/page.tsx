...
export default async function EscalationPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  if (order.current_state !== 'ESCALATION_ESCALATED' || !activeEscalation) {
    return (
      <div className="mx-auto max-w-md space-y-4">
        <PageHeader icon={Clock3} title="التصعيد" />
        <EmptyState icon={Clock3} message="لا يوجد تصعيد بلا رد يحتاج قراراً نهائياً حالياً." />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-md space-y-4">
      <PageHeader icon={Clock3} title="قرار نهائي للتصعيد" />
      ...
      {activeEscalation.actor === 'CUSTOMER_APPROVAL' ? (
        <div className="rounded-xl border border-red-200 bg-white p-5 shadow-sm">
          <h2 className="mb-3 flex items-center gap-2 font-semibold text-red-900">
            ...
            عدم رد العميل — إلغاء الاتفاقية (يفتح مسار تجديد)
          </h2>
