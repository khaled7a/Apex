...
export async function customerResponds(_prevState: ActionState, formData: FormData): Promise<ActionState> {
  const orderId = formString(formData, 'orderId');
  const expectedStateVersion = formInt(formData, 'expectedStateVersion');
  try {
    await apiFetch(`/escalation/${orderId}/customer-responds`, { method: 'POST', body: { expectedStateVersion } });
  } catch (err) {
