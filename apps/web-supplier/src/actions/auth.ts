...
export async function login(_prevState: AuthFormState, formData: FormData): Promise<AuthFormState> {
  ...
  try {
    session = await apiFetch<{ token: string; refreshToken: string }>('/auth/supplier/login', { method: 'POST', body: { email, password }, token: null });
  } catch (err) {
...
export async function logout(): Promise<void> {
  ...
  if (refreshToken) {
    await apiFetch('/auth/supplier/logout', { method: 'POST', body: { refreshToken }, token: null }).catch(() => undefined);
  }
